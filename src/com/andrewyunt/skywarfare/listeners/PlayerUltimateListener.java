...
public class PlayerUltimateListener implements Listener {
	...
	public void EPC(EntityDamageByEntityEvent event) {
		...
		try {
			damagedGP = SkyWarfare.getInstance().getPlayerManager().getPlayer(damaged.getName());
		} catch (PlayerException e) {
		}
		...
		if (event.getDamager() instanceof Player) {
			...
			try{
				gpDamager = SkyWarfare.getInstance().getPlayerManager().getPlayer(damager.getName());
			} catch(PlayerException e) {
				return;
			}
