package com.andrewyunt.warfare.utilities.command;
        ...
        if (args.length < 1) {
            ...
            for (final CommandArgument argument : this.arguments) {
                ...
                if (permission == null || sender.hasPermission(permission)) {
                    ...
                    HoverEvent hoverEvent = new HoverEvent(HoverEvent.Action.SHOW_TEXT, TextComponent.fromLegacyText(ChatColor.YELLOW + "Click to run " + ChatColor.GRAY + argument.getUsage(label)));
                    BaseComponent[] components = new ComponentBuilder(argument.getUsage(command.getName())).event(clickEvent).event(hoverEvent).append(" - " + argument.getDescription()).event(clickEvent).event(hoverEvent).create();
